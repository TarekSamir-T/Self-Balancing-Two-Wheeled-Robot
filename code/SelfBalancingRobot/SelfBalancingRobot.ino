...
int acc_calibration_value = 1044;  //needs calibration                            //Enter the accelerometer calibration value

//Various settings
float pid_p_gain = 25;         //8.5                                       //Gain setting for the P-controller (15)
float pid_i_gain = 1.1;       //0.4                                      //Gain setting for the I-controller (1.5)
float pid_d_gain = 0;         //4                                       //Gain setting for the D-controller (30)
float turning_speed = 15;     //15                                    //Turning speed (20)
float max_target_speed = 50;  //50                                //Max target speed (100)
...
void loop() {
  ...
  angle_gyro -= gyro_yaw_data_raw * 0.0000003;  //may be negative                            //Compensate the gyro offset when the robot is rotating
  ...
  if (received_byte & B00010000) {  //If the fifth bit of the receive byte is set deactivate the robot and reset everything
    pid_output = 0;                 //Set the PID controller output to 0 so the motors stop moving
    pid_i_mem = 0;                  //Reset the I-controller memory
    start = 0;                      //Set the start variable to 0
  }

  if (received_byte & B00100000) {  //If the sixth bit of the receive byte is set lower the target speed to a more efficient values
  ...
  if (received_byte & B01000000) {  //If the seventh bit of the receive byte is set increase the target speed
    float turning_speed = 25;       //this mode is best for more torque demanding tasks (ramps and fast movements)
...
ISR(TIMER2_COMPA_vect) {
  ...
  if (throttle_counter_left_motor > throttle_left_motor_memory) {    //If the number of loops is larger then the throttle_left_motor_memory variable
    ...
    if (throttle_left_motor_memory < 0) {                            //If the throttle_left_motor_memory is negative
      PORTD &= 0b11101111;                                           //Set output 3 low to reverse the direction of the stepper controller
      throttle_left_motor_memory *= -1;                              //Invert the throttle_left_motor_memory variable
    } else PORTD |= 0b00010000;                                      //Set output 3 high for a forward direction of the stepper motor
  } else if (throttle_counter_left_motor == 1) PORTD |= 0b00100000;  //Set output 2 high to create a pulse for the stepper controller
  else if (throttle_counter_left_motor == 2) PORTD &= 0b11011111;    //Set output 2 low because the pulse only has to last for 20us
